...
var typeTarget;
var typeLock = 0;
Recorder.inputTypes = ["text", "password", "file", "datetime", "datetime-local", "date", "month", "time", "week", "number", "range", "email", "url", "search", "tel", "color", "select-one"];
Recorder.tagNames = ["option"];
                    ...
                    if (enterTarget != null) {
                        ...
                        if (formChk == 'form' && (tempTarget.hasAttribute("id") || tempTarget.hasAttribute("name")) && (!tempTarget.hasAttribute("onsubmit"))) {
                            if (tempTarget.hasAttribute("id"))
                                this.record("submit", [
                                    {value: "id=" + tempTarget.id, type: "id"}
                                ], "");
                            else if (tempTarget.hasAttribute("name"))
                                this.record("submit", [
                                    {value: "name=" + tempTarget.name, type: "name"}
                                ], "");
                        } else
...
Recorder.addEventHandler('click', 'click', function(event) {
    ...
    if (event.button == 0 && !preventClick && event.isTrusted && Recorder.inputTypes.indexOf(type) < 0 && Recorder.tagNames.indexOf(tag) < 0) {
...
Recorder.addEventHandler('doubleClick', 'dblclick', function(event) {
    var type = event.target.type;
    var tag = event.target.tagName.toLowerCase();
    if (event.button == 0 && event.isTrusted && Recorder.inputTypes.indexOf(type) < 0 && Recorder.tagNames.indexOf(tag) < 0) {
...
Recorder.addEventHandler('dragAndDrop', 'mouseup', function(event) {
    ...
    if (this.selectMousedown && Recorder.tagNames.indexOf(tag) < 0) {
 ...
 * 
 * This is to populate more commands in DOM context menu 
 */
// © Ming-Hung Hsu, SideeX Team
Recorder.addEventHandler('mousedown', 'mousedown', function(event) {
    if(event.which == 3){
        var self = this;
        var myPort = browser.runtime.connect();

        var tmpLocators = this.locatorBuilders.buildAll(event.target);

        var contextMenuData = [];
        var tempElement = {};
        contextMenuData = [{
            command: "open",
            target: event.target.ownerDocument.URL
        },{
            command: "Title",
            target: normalizeSpaces(event.target.ownerDocument.title)
        }];

        for (var func in ConditionalContextMenu) {
            if(tmpLocators != undefined && tmpLocators[0] != undefined){    
                tempElement = ConditionalContextMenu[func](event.target, tmpLocators[0]);
                if(tempElement.target != undefined){
                    contextMenuData.push(tempElement);
                }
            }
        }

        if(tmpLocators != undefined && tmpLocators[0] != undefined){
            myPort.postMessage({ contextMenu: contextMenuData });
        }else{
            myPort.postMessage({ removeContextMenu: true });
        }

        myPort.onMessage.addListener(function portListener(m) {
            contextMenuData.forEach(function(type) {
                if(m.cmd.includes(type.command)){
                    if(!m.cmd.includes("store")){
                        if(m.cmd.includes("Title")){
                            self.record(m.cmd, [
                                {value: type.target, type: "system"}
                            ], '');
                        }else{
                            self.record(m.cmd, tmpLocators, (type.value!=undefined? type.value:''));
                        }
                    }else if(m.cmd.includes("store")){
                        if(m.cmd.includes("Title")){
                            self.record(m.cmd, [
                                {value: getInputValue(type.target), type: "system"}
                            ], '');
                        }else{
                            self.record(m.cmd, tmpLocators, getInputValue(event.target));
                        }
                    }
                }
            });
...
/* Value */
ConditionalContextMenu.value = function(element, target){
...
/* Text */
ConditionalContextMenu.text = function(element, target){
    var result = {command: "Text"}
    if (element && getText(element)!="") {
        result.target = target.value;
        result.value = getText(element);
    }
    return result;
}

/* Table */
ConditionalContextMenu.table = function(element, target){
    var result = {command: "Table"};
...
/* ElementPresent */
ConditionalContextMenu.elementPresent = function(element, target){
    var result = {command: "ElementPresent"};
    if (element) {
        result.target = target.value;
    }
    return result;
...
/* FontSize */
ConditionalContextMenu.fontSize = function(element, target){
    var result = {command: "FontSize" };
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("font-size");
    if (theCSSpropValue != "") {
        result.target = target.value;
        result.value = theCSSpropValue;
    }
    return result;
}

/* Color */
ConditionalContextMenu.color = function(element, target){
    var result = {command: "Color"};
    var colorValue = window.getComputedStyle(element,null).getPropertyValue("color");
    if (colorValue != "") {
        var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("color").split(',');
        theCSSpropValue[0] = theCSSpropValue[0].split('(');
        theCSSpropValue[2] = theCSSpropValue[2].split(')');
        var colorValue = rgbToHex(parseInt(theCSSpropValue[0][1]),parseInt(theCSSpropValue[1]), parseInt(theCSSpropValue[2][0]));                
        result.target = target.value;
        result.value = colorValue.toUpperCase();
    }
    return result;
}

/* FontFamily */
ConditionalContextMenu.fontFamily = function(element, target){
    var result = {command: "FontFamily"};
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("font-family");
    if (theCSSpropValue != "") {
        result.target = target.value;
        result.value = theCSSpropValue;
    }
...
/* FontStyle */
ConditionalContextMenu.fontStyle = function(element, target){
    var result = {command: "FontStyle"};
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("font-style");		
    if (theCSSpropValue != "") {
        result.target = target.value;
        result.value = theCSSpropValue;
    }
...
/* FontWeight */
ConditionalContextMenu.fontWeight = function(element, target){
    var result = {command: "FontWeight"};
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("font-weight");		
    if (theCSSpropValue != "") {
        result.target = target.value;
        result.value = theCSSpropValue;
    }
...
/* BorderColor */
ConditionalContextMenu.borderColor = function(element, target){
    var result = {command: "BorderColor"};
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("border-color");		
    if (theCSSpropValue != "") {
        if(theCSSpropValue.substr(0,3).match('rgb')) {
            var theCSSpropValue = theCSSpropValue.split(',');
            theCSSpropValue[0] = theCSSpropValue[0].split('(');
            theCSSpropValue[2] = theCSSpropValue[2].split(')');
            var colorValue = rgbToHex(parseInt(theCSSpropValue[0][1]),parseInt(theCSSpropValue[1]), parseInt(theCSSpropValue[2][0]));                
            result.target = target.value;
...
/* BackgroundColor */
ConditionalContextMenu.backgroundColor = function(element, target){
    var result = {command: "BackgroundColor"};
    var theCSSpropValue = window.getComputedStyle(element,null).getPropertyValue("background-color");		
	if (theCSSpropValue != "") {
		result.target = target.value;		
		if(theCSSpropValue.substr(0,3).match('rgb')) {
	        var theCSSpropValue = theCSSpropValue.split(',');
