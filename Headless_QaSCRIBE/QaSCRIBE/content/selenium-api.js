...
function Selenium(browserbot) {
    ...
    this.commandSpeedInterval = Selenium.DEFAULT_COMMAND_EXECUTION_SPEED;
...
Selenium.DEFAULT_COMMAND_EXECUTION_SPEED = 0;
Selenium.VISIBILITY_CHECK_INDICATOR = false;
...
Selenium.prototype.doReset = function() {
    this.defaultTimeout = Selenium.DEFAULT_TIMEOUT;
    eureQaObj.updateTimeOut(this.defaultTimeout);
...
/**
 * eureQa team modifed code
 * 
 * This code stringify Date object to sendMessage
 * as message passing api dosent transfer direct functions
 */
Selenium.prototype.doStore = function(value, varName) {
    if(Object.prototype.toString.call(value) == '[object Date]'){
        value = JSON.stringify(value);
        storedVars[varName] = value;
        return browser.runtime.sendMessage({ "storeStr": value, "storeVar": varName, "dateFunction": true}).then(function(){
            return Promise.resolve();
        });
    }else{
        storedVars[varName] = value;
        return browser.runtime.sendMessage({ "storeStr": value, "storeVar": varName }).then(function(){
            return Promise.resolve();
        });
    }
};
...
/* New prototypes introduced by Sideex Team - Ends Here */
...
Selenium.prototype.doClick = function(locator) {
     ...
     * eureQa team modified code
     * 
     * This below code implement visibility check option 
     */
    if(Selenium.VISIBILITY_CHECK_INDICATOR){
 ...
 * 
 * QaSCRIBE removed keypress commands
 */
 ...
 * eureQa team comented code
 * 
 * QaSCRIBE removed TypeKeys commands
 */
...
Selenium.prototype.doSendKeys = function(locator, value) {
    ...
    var element = this.browserbot.findElement(locator);
    var modifiedValue = preprocessParameter(value);

    if(Selenium.VISIBILITY_CHECK_INDICATOR){
        if(this.isVisible(locator)){
            if (modifiedValue.match(/[\uE000-\uF8FF]/)) {
                //we have special keys, process separately
                var keysRa = modifiedValue.split(/([\0-\uDFFF]+)|([\uE000-\uF8FF])/).filter(function(key) {
                ...
                bot.action.type(element, modifiedValue);
            }
        } else {
            throw new SeleniumError("Element Not Found"); 
        }
...
Selenium.prototype.doSetSpeed = function(value) {
     ...
     * eureQa team modified code
     * 
     * To set command execution speed in the component by passing message
     */
    ...
    if(value){
        this.commandSpeedInterval = value;  
        browser.runtime.sendMessage({
            setCommandSpeedValue: this.commandSpeedInterval
        })
    }else{
        throw new SeleniumError("Invalid speed interval");
    }
};
...
Selenium.prototype.getSpeed = function() {
    ...
    /** 
     * eureQa team comented and added code
     * 
     * This returns the current set command execution speed
     */
    //throw new SeleniumError("this operation is only implemented in selenium-rc, and should never result in a request making it across the wire");
    return this.commandSpeedInterval;
};
...
Selenium.prototype.isAlertPresent = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the alets present state
     */
    return this.browserbot.getAlertMessage(false).then(function(response) {
...
Selenium.prototype.isPromptPresent = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the prompts present state
     */
    return this.browserbot.getPromptMessage(false).then(function(response) {
        return Promise.resolve(response.status);
    },function(error){
        return Promise.resolve(error.status);
    });
...
Selenium.prototype.isConfirmationPresent = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the confirmation present state
     */
    return this.browserbot.getConfirmationMessage(false).then(function(response) {
        return Promise.resolve(response.status);
    },function(error){
        return Promise.resolve(error.status);
    });
...
Selenium.prototype.getAlert = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the alets and its messages
     */
    return this.browserbot.getAlertMessage(true).then(function(response) {
...
Selenium.prototype.getConfirmation = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the confirmation and its messages
     */
    return this.browserbot.getConfirmationMessage(true).then(function(response) {
        return Promise.resolve(response.message);
    },function(error){
        return Promise.resolve(error.message);
    });
...
Selenium.prototype.getPrompt = function() {
     ...
     * eureQa team modified code
     * 
     * This returns the prompts and its messages
     */
    return this.browserbot.getPromptMessage(true).then(function(response) {
        return Promise.resolve(response.message);
    },function(error){
        return Promise.resolve(error.message);
    });
...
Selenium.prototype.isVisible = function(locator) {
    ...
    var element;
    /**
     * eureQa team changed code to mimic CE engine behaviour
     */
    //element = this.browserbot.findElement(locator);
    element = this.browserbot.findElementOrNull(locator);
    if (element == null) {
        return false;
    }else{
        if (element.tagName) {
...
Selenium.prototype.doRunScript = function(script) {
    ...
    return this.browserbot.getRunScriptMessage().then(function(actualMessage) {
        return Promise.resolve(actualMessage);
    },function(error){
        return Promise.resolve(actualMessage);
    });
}
...
/* QaSCRIBE Implemented new commands for CSS and other HTML UI Elements - Starts Here.*/
/* Common Functions */
...
function formatFloatCalculations(I) {
...
var eureQaObj = {};
eureQaObj.RGB2Hex = function(theCSSpropValue){
    if(theCSSpropValue.substr(0,3).match('rgb')) {
        theCSSpropValue = theCSSpropValue.split(',')
        theCSSpropValue[0] = theCSSpropValue[0].split('(');
        theCSSpropValue[2] = theCSSpropValue[2].split(')');
        return rgbToHex(parseInt(theCSSpropValue[0][1]),parseInt(theCSSpropValue[1]), parseInt(theCSSpropValue[2][0])).toUpperCase();
    }else{
        return theCSSpropValue.toUpperCase();
    } 
};

eureQaObj.computedStyle = function(element, pseudo, property){
    var value = window.getComputedStyle(element,pseudo).getPropertyValue(property);
    if(property.toLowerCase().match(/color/) != null){                                                
        if(property.match(/border-color/) != null){
            return eureQaObj.computedBorderStyle(element, pseudo, property);
        }else{
            return [eureQaObj.RGB2Hex(value)];
        }
    }else{
        if(property.match(/border-width/) != null 
            || property.match(/border-style/) != null
            || property.match(/border-radius/) != null){
            return eureQaObj.computedBorderStyle(element, pseudo, property);
        }else{
            return [value];
        }
...
eureQaObj.computedBorderStyle = function(element, pseudo, property){
    var subProperty = property.split('-');
    if(subProperty[1] == "radius"){
        return [eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-top-left-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-top-right-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-bottom-right-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-bottom-left-"+subProperty[1]))];
    }else{
        return [eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-top-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-right-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-bottom-"+subProperty[1])),
                eureQaObj.RGB2Hex(window.getComputedStyle(element,pseudo).getPropertyValue(subProperty[0]+"-left-"+subProperty[1]))];
    }
};

eureQaObj.updateTimeOut = function(timeout){
    browser.runtime.sendMessage({
...
Selenium.prototype.doConcatenate = function(expression, variable) {
	var str;
	var concatedText="";
	if(expression.match(/,/)){
		//str = expression.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g);//split string based on comma and ignore commas with in double quotes
		str = expression.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);//split string based on comma and ignore commas with in double quotes
		for(var i = 0; i< str.length; i++ ){
			if (str[i].indexOf("\"") == 0 && str[i].lastIndexOf("\"") == (str[i].length-1) ) {
			  str[i] = str[i].substring(1, str[i].length - 1);
			}
			var text = str[i];
			concatedText += text
		}
		//concatedText = concatedText.replace(/\"/g,"");
	}else{
	concatedText = expression.replace(/^"(.+(?="$))"$/, '$1');
	}
    //storedVars[variable] = concatedText;
    this.doStore(concatedText, variable);
};

Selenium.prototype.doSum = function(expression, variable) {
	var re = /[ ,]*"([^"]+)"|([^,]+)/g;
	var matchValue;
	var str = expression;
	var SumText=0;
	while (matchValue = re.exec(str)) {
		var numre = /^[0-9]\d*(\.\d+)?$/;       /*PRO-8032 - Updated regexpr to accept decimals*/
		numre.exec(matchValue[2]);
		var num = numre.exec(matchValue[2]);
		var num1 = parseFloat(num);           /*PRO-8032 - Added decimals*/
		SumText = SumText + num1;
	}
	if(isNaN(SumText) ) {
		throw new SeleniumError("QaSCRIBE doesn't allow strings!");	
	}
    str=SumText;
    //storedVars[variable] = formatFloatCalculations(SumText);
    this.doStore(formatFloatCalculations(SumText), variable);
}

Selenium.prototype.doProduct = function(expression, variable) {
	var re = /[ ,]*"([^"]+)"|([^,]+)/g;
	var matchValue;
	var str = expression;
	var SumText=1;
...
Selenium.prototype.doAverage = function(expression, variable) {
	var re = /[ ,]*"([^"]+)"|([^,]+)/g;
	var matchValue;
	var count=0;
	var str = expression;
	...
	var st = formatFloatCalculations(SumText/count);

    //storedVars[variable] = st;
    this.doStore(st, variable);
}
...
Selenium.prototype.getColor = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'color')[0];
};

Selenium.prototype.getFontSize = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'font-size')[0]; 
};

Selenium.prototype.getHeight = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'height')[0]; 
};

Selenium.prototype.getFontFamily = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'font-family')[0]; 
};

Selenium.prototype.getFontStyle = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'font-style')[0]; 
};

Selenium.prototype.getFontWeight = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'font-weight')[0]; 
};

Selenium.prototype.getTextAlign = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'text-align')[0]; 
};

Selenium.prototype.getTextDecoration = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'text-decoration')[0]; 
};

Selenium.prototype.getTextOrientation = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'text-orientation')[0]; 
};

Selenium.prototype.getDisplay = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'display')[0]; 
};
...
Selenium.prototype.getPosition = function(locator) {
	...
	return eureQaObj.computedStyle(element, null, 'position')[0]; 
};

Selenium.prototype.getFloat = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'float')[0]; 
};

Selenium.prototype.getVerticalAlign = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'vertical-align')[0]; 
};

Selenium.prototype.getOpacity = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'opacity')[0]; 
};

Selenium.prototype.getPaddingLeft = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'padding-left')[0]; 
};

Selenium.prototype.getPaddingTop = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'padding-top')[0]; 
};

Selenium.prototype.getPaddingRight = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'padding-right')[0]; 
};

Selenium.prototype.getPaddingBottom = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'padding-bottom')[0]; 
};

Selenium.prototype.getMarginLeft = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'margin-left')[0]; 
};

Selenium.prototype.getMarginTop = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'margin-top')[0]; 
};

Selenium.prototype.getMarginRight = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'margin-right')[0]; 
};

Selenium.prototype.getMarginBottom = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'margin-bottom')[0]; 
};

Selenium.prototype.getBorderColor = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'border-color');
};

Selenium.prototype.getBorderRadius = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'border-radius'); 
};

Selenium.prototype.getBorderStyle = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'border-style'); 
};

Selenium.prototype.getBorderWidth = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'border-width'); 
};

Selenium.prototype.getWidth = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'width')[0]; 
};

Selenium.prototype.getOverflow = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'overflow')[0]; 
};

Selenium.prototype.getBackgroundColor = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'background-color')[0];    
};

Selenium.prototype.getBackgroundImage = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'background-image')[0]; 
};

Selenium.prototype.getBackgroundPosition = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'background-position')[0]; 
};

Selenium.prototype.getBackgroundRepeat = function(locator) {
	var element = this.browserbot.findElement(locator);
	if (!element) {
		throw new SeleniumError("Element not Found/Invalid");            
	}
	return eureQaObj.computedStyle(element, null, 'background-repeat')[0]; 
};
...
//For PRO-3542 doubleClickType
Selenium.prototype.doDoubleClickType = function(l, v){
	var element = this.browserbot.findElement(l);
	this.browserbot.doubleClickElement1(element, v);

};

Selenium.prototype.getTimeStamp = function(){
  now = new Date();
 ...
 var totally = year + month  + day + hour + minute + second;
//alert("timestamp"+year + "-" + month + "-" + day + " " + hour + ":" + minute + ":" + second);
  return totally;
}

Selenium.prototype.getRandomNumber = function(tar) {
	if(parseInt(tar) >= 10) 		throw new SeleniumError("Target value should be less than 10"); 
	if(parseInt(tar) <= 9)   {        
	var num = Math.random()*10000000000000000;
	var res = num.toString().substring(0,parseInt(tar));
//alert("num:::"+num)
	return res;
	}else{
throw new SeleniumError("Target value should be Integer"); 
	}
}

Selenium.prototype.doScrollToView = function(targ){
	var element = this.browserbot.findElement(targ);
	element.scrollIntoView();
}

Selenium.prototype.doClear = function(targ){
	var element = this.browserbot.findElement(targ);
    
    /**
     * eureQa team modified code
     * 
     * This code implement visibility check option 
     */
    if(Selenium.VISIBILITY_CHECK_INDICATOR){
        if(this.isVisible(targ)){
            element.value = "";
        } else {
            throw new SeleniumError("Element Not Found"); 
        } 
...
Selenium.prototype.getStyleAttribute = function(locator) {
	...
	return eureQaObj.computedStyle(element, null, attributeName)[0]; 
};

Selenium.prototype.doSendKeysEnter = function(locator, value) {
    /**
    * *Experimental* Simulates keystroke events on the specified element, as though you typed the value key-by-key.
    *
    * <p>This simulates a real user typing every character in the specified string and pressing ENTER key; it is also bound by the limitations of a
    * real user, like not being able to type into a invisible or read only elements. This is useful for dynamic UI widgets
    * (like auto-completing combo boxes) that require explicit key events.</p>
    * <p>Unlike the simple "type" command, which forces the specified value into the page directly, this command will not
    * replace the existing content. If you want to replace the existing contents, you need to use the simple "type" command to set the value of the
    ...
    * @param value the value to type
    */
    this.doSendKeys(locator, value);
    bot.action.pressEnterAfterSendKeys();
  };
  
  Selenium.prototype.doSendKeysTabOut = function(locator, value) {
    /**
    * *Experimental* Simulates keystroke events on the specified element, as though you typed the value key-by-key.
    *
    * <p>This simulates a real user typing every character in the specified string and pressing TAB key; it is also bound by the limitations of a
    * real user, like not being able to type into a invisible or read only elements. This is useful for dynamic UI widgets
    * (like auto-completing combo boxes) that require explicit key events.</p>
    * <p>Unlike the simple "type" command, which forces the specified value into the page directly, this command will not
    * replace the existing content. If you want to replace the existing contents, you need to use the simple "type" command to set the value of the
    ...
    */
    this.doSendKeys(locator, value);
    bot.action.pressTabOutAfterSendKeys();
  };

/* QaSCRIBE Implemented new commands for CSS and other HTML UI Elements - Ends Here. */

var nonWaitActions = ['open'
    , 'selectWindow'
...
//var noPrepopulateCommands = [];
var eureqaCommandBuilder = [];

for (var func in Selenium.prototype) {
      var r;
    if(!func.match(/getWhetherThis[\s\S]+?Expression/) && !func.match(/getTimeStamp/)){
        if (func.match(/^do[A-Z]/)) {
            var action = func.substr(2, 1).toLowerCase() + func.substr(3);
            //allAvaliableCommands.push(action);
            if (!action.match(/^waitFor/) && nonWaitActions.indexOf(action) < 0) {
                //allAvaliableCommands.push(action + "AndWait");
                eureqaCommandBuilder.push({method:func,command:action + "AndWait"});
            }
        } else if (func.match(/^assert.+/)) {
            //allAvaliableCommands.push(func);
            eureqaCommandBuilder.push({method:func,command:func});
            //allAvaliableCommands.push("verify" + func.substr(6));
            eureqaCommandBuilder.push({method:func,command:"verify" + func.substr(6)});
        } else if ((r = func.match(/^(get|is)(.+)$/))) {
            var base = r[2];
            //allAvaliableCommands.push("assert" + base);
            //allAvaliableCommands.push("verify" + base);
            //allAvaliableCommands.push("store" + base);
            //allAvaliableCommands.push("waitFor" + base);
            eureqaCommandBuilder.push({method:func,command:"assert" + base});
            eureqaCommandBuilder.push({method:func,command:"verify" + base});
            eureqaCommandBuilder.push({method:func,command:"store" + base});
            eureqaCommandBuilder.push({method:func,command:"waitFor" + base});

            var r2;
            if ((r = func.match(/^is(.*)Present$/))) {
                base = r[1];
                //allAvaliableCommands.push("assert" + base + "NotPresent");
                //allAvaliableCommands.push("verify" + base + "NotPresent");
                //allAvaliableCommands.push("waitFor" + base + "NotPresent");

                eureqaCommandBuilder.push({method:func,command:"assert" + base + "NotPresent"});
                eureqaCommandBuilder.push({method:func,command:"verify" + base + "NotPresent"});
                eureqaCommandBuilder.push({method:func,command:"waitFor" + base + "NotPresent"});


            } else {
                //allAvaliableCommands.push("assertNot" + base);
                //allAvaliableCommands.push("verifyNot" + base);
                //allAvaliableCommands.push("waitForNot" + base);

                eureqaCommandBuilder.push({method:func,command:"assertNot" + base});
                eureqaCommandBuilder.push({method:func,command:"verifyNot" + base});
                eureqaCommandBuilder.push({method:func,command:"waitForNot" + base});
...
//allAvaliableCommands.push("pause");
/**
 * All the TimeStamp  commands are removed except storeTimeStamp command 
 */
//allAvaliableCommands.push("storeTimeStamp");
eureqaCommandBuilder.push({method:"getTimeStamp",command:"storeTimeStamp"});

/**
 * eureQa team added code
 * 
 * Visibility check functionality 
 */
browser.runtime.onMessage.addListener(function(message, sender, response){
    if(message.visibilityCheckIndicator || message.defaultTimeout || message.locatorOrder){
        if(message.visibilityCheckIndicator != null){
            Selenium.VISIBILITY_CHECK_INDICATOR = message.visibilityCheck;
        }
        if(message.defaultTimeout != null){
            Selenium.DEFAULT_TIMEOUT = message.defaultTimeout;
        }
        if(message.locatorOrder){
            LocatorBuilders.order = message.locatorOrder;
        }
        return Promise.resolve(); 
    }
    //Store json response in a variable
    else if(message.storeStr){
        storedVars[message.storeStr] = message.storeVar;
        return Promise.resolve(); 
    }
});