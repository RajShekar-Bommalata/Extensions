                ...
                switch (event.data.recordedType) {
                    case "prompt":
                        if (event.data.recordedResult != null) {
                            recorder.record("answerOnNextPrompt", [
                                {value: event.data.recordedResult, type: "system"}
                            ], "", true, event.data.frameLocation);
                        } else {
                            recorder.record("chooseCancelOnNextPrompt", [
                                {value: "", type: "system"}
                            ], "", true, event.data.frameLocation);
                        }
                        recorder.record("assertPrompt", [
                            {value: event.data.recordedMessage, type: "system"}
                        ], "", false, event.data.frameLocation);
                        break;
                    case "confirm":
                        if (event.data.recordedResult == true) {
                            ...
                            recorder.record("chooseCancelOnNextConfirmation", [
                                {value: "", type: "system"}
                            ], "", true, event.data.frameLocation);
                        }
                        recorder.record("assertConfirmation", [
                            {value: event.data.recordedMessage, type: "system"}
                        ], "", false, event.data.frameLocation);
                        break;
                    case "alert":
                        //record("answerOnNextAlert",[[event.data.recordedResult]],"",true);
                        recorder.record("assertAlert", [
                            {value: event.data.recordedMessage, type: "system"}
                        ], "", false, event.data.frameLocation);
                        break;
                }
