...
BrowserBot.prototype._modifyWindow = function(win) {
     ...
     *
     * Condition added as the extension native Alerts/Prompts/Confirmations recorded and not visible on Component 
     */
    if(!(location.href.startsWith("moz-extension://") ||
...
/**
 * This function validates whether the locator contains Shadow DOM notation #SHADOWDOM# or @eureQa@
 * 
 * According to the notation the respective SHadow DOM logic will execute and
 * the result element is returned
 */
function checkAndReturnShadowDomLocator(locator){
    /* Default Shadow DOM logic */
    if(locator.indexOf("#SHADOWDOM#") != -1){
        var selectorsSequence = locator.split('#SHADOWDOM#');
        var cobj;
        var celement;
        var cDocument = document;
    
        for(i in selectorsSequence){
            if(i < selectorsSequence.length-1){
                celement = selectorsSequence[i].trim();
                cobj = cDocument.querySelector(celement).shadowRoot;
                cDocument = cobj;
            }
        }
        return cDocument.querySelector(selectorsSequence[selectorsSequence.length-1]);
    }
    
    /* Recursive Shadow DOM logic */
    if(locator.indexOf("@eureQa@") != -1){
        locator = locator.split('@eureQa@')[1].trim();
        var cobj;
        var shadowRootObj;
        var i = 0;

        /* Split locator at spaces to get list of element hirarchie */
        var subCollection = locator.trim().match(/(?=\S)[^"\s]*(?:"[^\\"]*(?:\\[\s\S][^\\"]*)*"[^"\s]*)*/g);
        shadowRootObj = document;
        cobj = shadowRootObj;
        while(i<subCollection.length){
            var detectItem = false;
            recursivefunction(cobj, cobj.children);
            
            /* Recursive Function to check for Shadow DOM nodes and assign to document if any found*/
            function recursivefunction(_elementObject, _children){
                if(i<subCollection.length && !detectItem){
                    if(_elementObject.shadowRoot){
                        shadowRootObj = _elementObject.shadowRoot;
                        cobj = shadowRootObj;
                        recursivefunction(cobj, cobj.children);
                    }
                    if(_children.length>0){
                        for (item of _children){
                            if(item.tagName.toLocaleLowerCase() !== "style" && item.tagName.toLocaleLowerCase() !== "script" && subCollection[i] && i<subCollection.length){

                                if(subCollection[i].indexOf("@exactMatch") != -1 ||
                                    subCollection[i].indexOf("@contains") != -1 ||
                                    subCollection[i].indexOf("@startsWith") != -1 ||
                                    subCollection[i].indexOf("@endsWith") != -1){

                                    /* Iterate throught nodes to extract the text from the specific node */    
                                    var itemText = "";
                                    for (var j = 0; j < item.childNodes.length; j++) {
                                        var curNode = item.childNodes[j];
                                        if (curNode.nodeType == Node.TEXT_NODE) {
                                            itemText = curNode.nodeValue;
                                        }
                                    }
                                    
                                    if(itemText != ""){
                                        if(subCollection[i].indexOf("[@exactMatch") != -1){
                                            var checkString = subCollection[i].match(/\[@exactMatch=(?:'|")(.*?)(?:'|")\]/)[1];
                                            checkString = checkString.replace(/[-[\]{}()*+?.,\\^$|#]/g, '\\$&');
                                            var regex = new RegExp('^'+checkString+'$', "g");
                                            if(itemText.match(regex)){
                                                subCollection[i] = subCollection[i].replace(/\[@exactMatch=(?:'|")(.*?)(?:'|")\]/, "");
                                                cobj = item;
                                                i++;
                                                detectItem = true;
                                            }
                                        }else if(subCollection[i].indexOf("[@contains") != -1){
                                            var checkString = subCollection[i].match(/\[@contains=(?:'|")(.*?)(?:'|")\]/)[1];
                                            checkString = checkString.replace(/[-[\]{}()*+?.,\\^$|#]/g, '\\$&');
                                            var regex = new RegExp(checkString, "g");
                                            if(itemText.match(regex)){
                                                subCollection[i] = subCollection[i].replace(/\[@contains=(?:'|")(.*?)(?:'|")\]/, "");
                                                cobj = item;
                                                i++;
                                                detectItem = true;
                                            }
                                        }else if(subCollection[i].indexOf("[@startsWith") != -1){
                                            var checkString = subCollection[i].match(/\[@startsWith=(?:'|")(.*?)(?:'|")\]/)[1];
                                            checkString = checkString.replace(/[-[\]{}()*+?.,\\^$|#]/g, '\\$&');
                                            var regex = new RegExp('^'+checkString, "g");
                                            if(itemText.match(regex)){
                                                subCollection[i] = subCollection[i].replace(/\[@startsWith=(?:'|")(.*?)(?:'|")\]/, "");
                                                cobj = item;
                                                i++;
                                                detectItem = true;
                                            }
                                        }else if(subCollection[i].indexOf("[@endsWith") != -1){
                                            var checkString = subCollection[i].match(/\[@endsWith=(?:'|")(.*?)(?:'|")\]/)[1];
                                            checkString = checkString.replace(/[-[\]{}()*+?.,\\^$|#]/g, '\\$&');
                                            var regex = new RegExp(checkString+"$", "g");
                                            if(itemText.match(regex)){
                                                subCollection[i] = subCollection[i].replace(/\[@endsWith=(?:'|")(.*?)(?:'|")\]/, "");
                                                cobj = item;
                                                i++;
                                                detectItem = true;
                                            }
                                    ...
                                    /* Recursive logic to find Parent/Adjacent/Sibling/Children elemnts */
                                    var subLocator = subCollection[i-1];
                                    if(subLocator && (subLocator.indexOf(":parent()") == 0 ||
                                        subLocator.indexOf(":adjacent()") == 0 ||
                                        subLocator.indexOf(":sibling(") == 0 ||
                                        subLocator.indexOf(":children(") == 0)){
                                        lookAhead();
                                    }
                                    function lookAhead(){
                                        if(subLocator.indexOf(":parent()") == 0 && item.parentElement){
                                            subLocator = subLocator.replace(/^:parent\(\)/, "");
                                            item = item.parentElement;
                                            cobj = item;
                                        }else if(subLocator.indexOf(":adjacent()") == 0){
                                            subLocator = subLocator.replace(/^:adjacent\(\)/, "");
                                            item = item.nextElementSibling;
                                            cobj = item;
                                        }else if(subLocator.indexOf(":sibling(") == 0){
                                            item = (item.parentElement == null ? shadowRootObj : item.parentElement); 
                                            subLocator = subLocator.match(/^:sibling\((.*?)\)/, "")[1];
                                            subLocator = subLocator.replace(/^:sibling\((.*?)\)/, "");
                                            cobj = item.querySelectorAll(subLocator)[0];
                                        }else if(subLocator.indexOf(":children(") == 0){
                                            subLocator = subLocator.match(/^:children\((.*?)\)/, "")[1];
                                            subLocator = subLocator.replace(/^:children\((.*?)\)/, "");
                                            cobj = item.querySelectorAll(":scope > "+subLocator)[0];
                                        }

                                        /* Call lookAhead recursive function if the locator is not completely iterated */
                                        if(subLocator.indexOf(":parent()") == 0 ||
                                            subLocator.indexOf(":adjacent()") == 0 ||
                                            subLocator.indexOf(":sibling(") == 0 ||
                                            subLocator.indexOf(":children(") == 0){
                                            lookAhead();
                                ...
                                /* Execute the locator as normal if no eureQa custom notation is present */
                                }else if(shadowRootObj.querySelectorAll(subCollection[i])[0]!==undefined){
                                    cobj = shadowRootObj.querySelectorAll(subCollection[i])[0];
                                    item = cobj;
                                    i++;
                                }

                                /* Check if the locator list is iterated and if not check for children or shadow node and call recursive function*/
                                if(i<subCollection.length && (item.children.length>0 || item.shadowRoot)){
                                    detectItem = false;
                                    recursivefunction(item, item.children);
                                }
                            }
                        }
                    }
        ...
        return ((cobj == undefined || cobj.tagName == undefined) ? null : cobj);
    }
}
...
BrowserBot.prototype.findElementOrNull = function(locator, win) {
    ...
    if(locator.indexOf("#SHADOWDOM#") != -1 ||
        locator.indexOf("@eureQa@") != -1){
        element = checkAndReturnShadowDomLocator(locator);
    }else{
        ...
        element = this.findElementBy(locator.type, locator.string, win.document, win);
    }
...
BrowserBot.prototype.findElement = function(locator, win) {
    ...
    if(locator.indexOf("#SHADOWDOM#") != -1 ||
        ...
        element = this.findElementOrNull(locator, win);
    }

    if (element == null) throw new SeleniumError("Element " + locator + " did not found");
 ...
 */
BrowserBot.prototype.findElementForFindButton = function(locator, win) {
    var element = null;
 ...
 * eureQa team modified code
 */
BrowserBot.prototype.getPromptMessage = function(_isMessage) {
    this.promptResponse = false;
    ...
    let response = new Promise(function(resolve, reject) {
        setTimeout(function() {
            if (!self.promptResponse) {
                reject({status:false, message:"Prompt not"});
            } else {
                if(self.promptMessage!=null){
                    resolve({status:self.promptResponse, message:self.promptMessage});
                }else{
 ...
 * eureQa team modified code
 */
BrowserBot.prototype.getConfirmationMessage = function(_isMessage) {
    this.confirmationResponse = false;
        ...
        setTimeout(function(){   
            if (!self.confirmationResponse) {
                ...
                if(self.confirmationMessage!=null){
                    resolve({status:self.confirmationResponse, message:self.confirmationMessage});
                }else{
                    resolve({status:false, message:""});
                }
                if(_isMessage){
 ...
 * eureQa team modified code
 */
BrowserBot.prototype.getAlertMessage = function(_isMessage) {
    let self = this;
    let response = new Promise(function(resolve, reject) {
        setTimeout(function(){   
            if (!self.alertResponse) {
                ...
                resolve({status:self.alertResponse, message:self.alertMessage});
                if(_isMessage){
